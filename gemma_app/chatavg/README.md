...
5.  **Переменные окружения (.env):**
    *   `CHATAVG_PORT`: Порт сервера (по умолчанию 8080).
    *   `CHATAVG_SECRET`: Ключ для подписи JWT (минимум 32 символа).
    *   `CHATAVG_ADMIN_PASSWORD`: Пароль администратора.
    *   `CHATAVG_TOKEN_EXPIRY`: Время жизни сессии (например, `7d`, `24h`).
    *   `CHATAVG_ALLOWED_ORIGINS`: Список разрешенных доменов для CORS (через запятую).
    *   `ALLOW_CUSTOM_PROVIDER_URLS`: Установите `true` для отключения защиты от SSRF (не рекомендуется).
