...
router.post('/users/:username', asyncHandler(async (req, res) => {
    ...
    if (data.n_ctx !== undefined)
        user.n_ctx = data.n_ctx;
    if (data.system_prompt !== undefined)
        user.system_prompt = data.system_prompt;
    await userRepository.save(username, user);
    AuditService.log(req.user.username, isNew ? 'USER_CREATE' : 'USER_UPDATE', { target_user: username }, req.ip || req.connection.remoteAddress);
...
router.delete('/users/:username', asyncHandler(async (req, res) => {
    ...
    AuditService.log(req.user.username, 'USER_DELETE', { target_user: username }, req.ip || req.connection.remoteAddress);
    res.json({ status: 'success' });
}));
// ── Categories ──────────────────────────────────────────
...
router.post('/categories/:category_name', asyncHandler(async (req, res) => {
    ...
    AuditService.log(req.user.username, 'CATEGORY_UPDATE', { target_category: catName }, req.ip || req.connection.remoteAddress);
    res.json({ status: 'success' });
}));
router.delete('/categories/:category_name', asyncHandler(async (req, res) => {
    ...
    AuditService.log(req.user.username, 'CATEGORY_DELETE', { target_category: catName }, req.ip || req.connection.remoteAddress);
    res.json({ status: 'success' });
}));
...
router.post('/categories/:category_name/test', asyncHandler(async (req, res) => {
    ...
    if (!provider)
        return res.status(500).json({ error: 'Провайдер не найден' });
