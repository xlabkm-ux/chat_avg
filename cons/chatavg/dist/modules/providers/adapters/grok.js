...
class GrokProvider extends OpenAICompatProvider {
    ...
    async *handleChat(messages, config, options) {
        ...
        if (collectionIds && Array.isArray(collectionIds) && collectionIds.length > 0) {
            ...
            const hasCollectionsTool = tools.some(t => t.type === 'collections_search' ||
                t.type === 'file_search' ||
                ...
                if (event.type === 'delta') {
                    ...
                    for (const tc of event.toolCall) {
                        ...
                        if (!toolCallDeltas[idx])
                            toolCallDeltas[idx] = { id: '', function: { name: '', arguments: '' } };
                        if (tc.id)
                            toolCallDeltas[idx].id = tc.id;
                        if (tc.function?.name)
                            toolCallDeltas[idx].function.name += tc.function.name;
                        if (tc.function?.arguments)
                            toolCallDeltas[idx].function.arguments += tc.function.arguments;
                    }
            ...
            if (finishReason === 'tool_calls' && toolCalls.length > 0) {
                ...
                for (const tc of toolCalls) {
                    if (tc.function.name === 'collections_search') {
                        ...
                        const results = await this._searchCollections(args.query, args.collection_ids || collectionIds, activeConfig.api_key);
                        currentMessages.push({
                            role: 'tool',
                            tool_call_id: tc.id,
                            content: results
