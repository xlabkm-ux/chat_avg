...
class MissionRepository {
  create(missionData) {
    ...
    stmt.run(
      id,
      missionData.sessionId,
      missionData.username,
      missionData.semanticProtocolId || null,
      missionData.glossaryVersion || null,
      missionData.mode || 'balanced',
      missionData.goal,
      JSON.stringify(missionData.constraints || []),
      JSON.stringify(missionData.openQuestions || []),
      JSON.stringify(missionData.context || {}),
      now,
      now
    );
  ...
  update(id, missionData, username) {
    ...
    if (missionData.goal !== undefined) { fields.push('goal = ?'); values.push(missionData.goal); }
    if (missionData.mode !== undefined) { fields.push('mode = ?'); values.push(missionData.mode); }
    if (missionData.constraints !== undefined) { fields.push('constraints = ?'); values.push(JSON.stringify(missionData.constraints)); }
